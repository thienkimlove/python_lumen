...
def proxy(url, country, agent, rand, site_name):
    credentials = None
    result = None
    ...
    if credentials is not None:
        ...
        if result is None:
            try:
                ...
                result = "{ 'error' : " + format(
                    err) + ", 'url' : " + url + ", 'agent' : " + agent + ", 'country' : " + country + ", 'rand' : " + rand + " }"
...
def virtual_curl(country, url, rand, agent, site_name, redirection=0):
    is_ok = True
    try:
        result = proxy(url, country, agent, rand, site_name)
        if not result:
            is_ok = False
    except Exception as err:
    ...
    if redirection < 3 and is_ok:
        ...
        if check is not None:
            ...
            return virtual_curl(country, check, rand, agent, site_name, redirection)
        else:
            special_check = regex_without_scheme(result, url)
            if special_check is not None:
                redirection = redirection + 1
                return virtual_curl(country, special_check, rand, agent, site_name, redirection)
    if isinstance(result, str) and len(result) > 2000 and is_ok:
...
def fetch_item(log_id, country, allow, link, site_name):
    ...
    response = virtual_curl(user_country, link, rand, agent_row, site_name)
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for log in logs:
            pool.spawn(fetch_item, log.id, log.country, log.allow, log.link, log.site_name)
        pool.join()
        end_time = time.time()
